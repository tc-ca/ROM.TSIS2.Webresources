...
if (lang == 1036) {
  markCompleteValidationTextLocalized =
    "Tous les champs indiqués par un '+' bleu doivent être complétés avant que la constatation puissent être marqué comme terminé.";
  ...
  missingFinalEnforcementActionTextLocalized =
    "Un ou plusieurs enregistrements de constatations sélectionnés n'ont pas de mesure d'application finale. Toutes les constatations sélectionnées doivent avoir une mesure d'application finale pour créer un rapport de constatations.";
  missingFinalEnforcementActionForActionCreateTextLocalized =
    "Un ou plusieurs enregistrements de constatations sélectionnés n'ont pas de mesure d'application finale. Toutes les constatations sélectionnées doivent avoir une mesure d'application finale.";
  createAcionFromObservationTitleLocalized = "Erreur de validation";
  createAcionFromObservationTextLocalized =
    "Il n'est pas possible de créer des mesures d'exécution pour les observations.";
  ...
  missingFinalEnforcementActionTextLocalized =
    "One of more selected Finding records do not have a Final Enforcement Action. All selected findings must have a Final Enforcement Action to create a Findings Report.";
  missingFinalEnforcementActionForActionCreateTextLocalized =
    "One of more selected Finding records do not have a Final Enforcement Action. All selected findings must have a Final Enforcement Action.";
...
let issoOperationTypeGuids = [
  "b27e5003-c751-eb11-a812-000d3af3ac0d",
  "c97a1a12-d8eb-eb11-bacb-000d3af4fbec",
  "21ca416a-431a-ec11-b6e7-000d3a09d067",
  "3b261029-c751-eb11-a812-000d3af3ac0d",
  "d883b39a-c751-eb11-a812-000d3af3ac0d",
  "da56fea1-c751-eb11-a812-000d3af3ac0d",
  "199e31ae-c751-eb11-a812-000d3af3ac0d",
];
...
function markComplete(primaryControl) {
  ...
  if (
    finalEnforcementAction != null &&
    (!issueaddressedonsiteVisibility || issueaddressedonsite != null) &&
    nonComplianceTimeframeCheck
  ) {
    primaryControl.getAttribute("statuscode").setValue(717750002); //Complete
    primaryControl.data.save().then(function success(result) {
...
async function isAvSecBusinessUnit() {
  ...
  let result = await Xrm.WebApi.retrieveMultipleRecords("businessunit", currentUserBusinessUnitFetchXML);
  if (result.entities.length > 0) {
    const userBusinessUnitId = result.entities[0].businessunitid;
    return await isUserInAvSecBU(userBusinessUnitId);
  }
  return false;
}
...
async function isISSOBusinessUnit() {
  ...
  if (result.entities.length > 0) {
    const userBusinessUnitId = result.entities[0].businessunitid;
    return await isUserInISSOBU(userBusinessUnitId);
  }
  return false;
}
...
async function isTCBusinessUnit() {
  ...
  if (result.entities.length > 0) {
    const userBusinessUnitId = result.entities[0].businessunitid;
    return await isUserInTCBU(userBusinessUnitId);
  }
  return false;
}
...
async function FindingsReport(findingGUIDs, primaryControl) {
  ...
  let { allFindingsHaveFinalEnforcementAction, aFindingIsProtectedB, aFindingIsObservation } =
    checkIfAllFindingsHaveEnforcementAction(findingRows);
...
function checkIfAllFindingsHaveEnforcementAction(findingRows) {
  ...
  findingRows.forEach(function (findingRow) {
    ...
    if (findingSensitivityLevel == 717750001) {
      //Protected B
      aFindingIsProtectedB = true;
    }
    if (findingType == 717750001) {
      // Observation
...
function getTypeOfEnforcementActionValueInEntity(highestEnforcementAction) {
  switch (highestEnforcementAction) {
    ...
    case 0 /* TBD: what enforcement action in finding make the Enforcement Action type to penal process */:
...
function AssignCorrectiveAction(findingGUIDs, primaryControl) {
  ...
  // Open the form
  Xrm.Navigation.openForm(correctiveActionFormOptions, findingGUIDs).then(function (success) {
    console.log(success);
    ...
    relatedFindings.forEach(
      function (finding) {
        ...
        Xrm.WebApi.createRecord("ts_correctiveactionfinding", data).then(function (success) {
          console.log(success);
        });
      },
      function (error) {
...
async function isNCAT(findingGUID) {
  let operationTypeFetchXml = [
    ...
    "<filter type='and'>",
    "<condition attribute='ovs_findingid' operator='eq' value='",
    findingGUID,
    "'/>",
    "</filter>",
    "</link-entity>",
...
async function createEnforcementAction(findingGUIDs, primaryControl) {
  ...
  if (!isAvSecBusinessUnit()) {
    if (!allFindingsHaveFinalEnforcementAction) {
      showAlertDialog(
        missingFinalEnforcementActionForActionCreateTextLocalized,
        missingFinalEnforcementActionTitleLocalized
      );
      return;
    }
  }
  ...
  if (currentEntityName === "incident") {
    ...
    try {
      ...
      const woServiceTask = await Xrm.WebApi.retrieveRecord(
        "msdyn_workorderservicetask",
        caseIdPrimary,
        "?$select=_ovs_caseid_value"
      );
    ...
    findingRows.forEach(function (findingRow) {
      ...
      if (
        !enforcementActionType.includes(
          getTypeOfActionValueInEntity(findingRow.getAttribute("ts_finalenforcementaction").getValue())
        )
      ) {
  ...
  if (lang == 1033) {
    ...
    confirmStrings = {
      text:
        toolName +
        " result from Finding " +
        completeFinding.ovs_finding +
        " will be applied to the selected Findings. Do you wish to proceed?",
      title: "Apply Result",
    };
  } else {
    ...
    confirmStrings = {
      text:
        ...
        completeFinding.ovs_finding +
        " vont être appliqués aux constatations sélectionnées. Voulez-vous continuer?",
    ...
    Xrm.Navigation.openConfirmDialog(confirmStrings, confirmOptions).then(async function (success) {
      if (success.confirmed) {
        ...
        if (completeFinding._ts_ncatactualorpotentialharm_value != null) {
          //NCAT
          data = {
            "ts_NCATActualorPotentialHarm@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncatactualorpotentialharm_value + ")",
            "ts_NCATIntentionality@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncatintentionality_value + ")",
            "ts_NCATComplianceHistory@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncatcompliancehistory_value + ")",
            "ts_NCATEconomicBenefit@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncateconomicbenefit_value + ")",
            "ts_NCATEconomicBenefit@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncateconomicbenefit_value + ")",
            "ts_NCATMitigationofNonCompliantBehaviors@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncatmitigationofnoncompliantbehaviors_value + ")",
            "ts_NCATCooperationwithInspectionorInvestigat@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncatcooperationwithinspectionorinvestigat_value + ")",
            "ts_NCATDetectionofNonCompliances@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ncatdetectionofnoncompliances_value + ")",
          ...
          data = {
            ...
            "ts_RATEGeneralComplianceHistory@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_rategeneralcompliancehistory_value + ")",
            "ts_RATEActualorPotentialHarm@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_rateactualorpotentialharm_value + ")",
            "ts_RATEIntentionality@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_rateintentionality_value + ")",
            "ts_RATEEconomicBenefit@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_rateeconomicbenefit_value + ")",
            "ts_RATEResponsibility@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_rateresponsibility_value + ")",
            "ts_RATEMitigationofNonCompliantBehaviors@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ratemitigationofnoncompliantbehaviors_value + ")",
            "ts_RATEPreventingRecurrence@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ratepreventingrecurrence_value + ")",
            "ts_RATECooperationwithInspectionorInvestigat@odata.bind":
              "/ts_assessmentratings(" + completeFinding._ts_ratecooperationwithinspectionorinvestigat_value + ")",
        ...
        for (let selectedFindingId of SelectedControlsSelectedItemIds) {
          if (selectedFindingId != completeFinding.ovs_findingid) {
            ...
            const finding = await Xrm.WebApi.retrieveRecord(
              "ovs_finding",
              selectedFindingId,
              "?$select=ts_findingtype"
            );
