...
async function isAvSecBusinessUnit() {
  ...
  return userBusinessUnitId && aviationBuGuid && userBusinessUnitId.toLowerCase() === aviationBuGuid.toLowerCase();
}
...
async function isTCBusinessUnit() {
  ...
  let userBusinessUnitName = await Xrm.WebApi.retrieveMultipleRecords("businessunit", currentUserBusinessUnitFetchXML);
  const userBusinessUnitId = userBusinessUnitName.entities[0].businessunitid;
  const tcBuGuid = await getEnvironmentVariableValue("ts_TCDirectorateBU");

  return userBusinessUnitId && tcBuGuid && userBusinessUnitId.toLowerCase() === tcBuGuid.toLowerCase();
}
