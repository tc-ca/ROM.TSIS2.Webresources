...
async function isTaskTypeCustomQuestionnaire(primaryControl) {
  var taskTypeAttribute = primaryControl.getAttribute("ts_tasktype").getValue();
...
async function surveyHasErrors(primaryControl) {
  ...
  var serviceTaskStartDate = primaryControl.getAttribute("ts_workorderservicetaskstartdate").getValue();
...
async function checkOperationRiskAssessment(formContext, survey) {
  const workOrderAttribute = formContext.getAttribute("ts_workorder").getValue();
...
async function hasSubmittedOperationRiskAssessmentForWorkOrder(workOrderId) {
  ...
  try {
    ...
    var result = await Xrm.WebApi.retrieveMultipleRecords("ts_operationriskassessment", fetchXml);

    return result.entities && result.entities.length > 0;
  } catch (e) {
    console.error("Error checking submitted Operation Risk Assessment for work order:", e);
    return false;
  }
}
...
function completeConfirmation(formContext, survey) {
  ...
  Xrm.Navigation.openConfirmDialog(confirmStrings, confirmOptions).then(function (success) {
    if (success.confirmed) {
      formContext.getAttribute("ts_percentcomplete").setValue(100.0);
      //Set Status Reason to Complete
      formContext.getAttribute("statuscode").setValue(741130001);
      //Set Service Task End Date to current date
      formContext.getAttribute("ts_workorderservicetaskenddate").setValue(new Date());
    ...
    Xrm.WebApi.retrieveRecord(
      ...
      .then(function (result) {
        var relatedId =
          ...
          (result.ts_workorderservicetask ? result.ts_workorderservicetask.msdyn_workorderservicetaskid : null);
