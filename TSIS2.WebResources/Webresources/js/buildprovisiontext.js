...
async function gatherAncestorProvisionText(provision, lang) {
  ...
  //Base case. If provision has no parent, just return its own text
  if (!provision._qm_rcparentlegislationid_value) {
  ...
  if (provisionType == "8726bb2a-497c-eb11-a812-000d3af31ad8") {
    return (
      (await gatherAncestorProvisionText(parent, lang)) +
      `<strong><a href='#' onclick="navigateToProvision('${provision.qm_rclegislationid}')">${provisionText}</a></strong></br>`
    );
  }

  return (
    (await gatherAncestorProvisionText(parent, lang)) +
    `<strong><a href='#' onclick="navigateToProvision('${provision.qm_rclegislationid}')">${provision.qm_name}</a></strong>: ${provisionText}</br>`
  );
}
...
async function gatherDescendentProvisionText(provision, lang) {
  ...
  for (var i in children) {
    ...
    if (provisionType == "8726bb2a-497c-eb11-a812-000d3af31ad8" || children[i].qm_legislationlbl == null) {
      provisionText +=
        `<li><strong><a href='#' onclick="navigateToProvision('${children[i].qm_rclegislationid}')">${childText}</a></strong></li>` +
        (await gatherDescendentProvisionText(children[i], lang));
    } else {
      provisionText +=
        `<li><strong><a href='#' onclick="navigateToProvision('${children[i].qm_rclegislationid}')">${children[i].qm_legislationlbl}</a></strong> ${childText}</li>` +
        (await gatherDescendentProvisionText(children[i], lang));
    }
  }
  provisionText += "</ul>";
...
async function getParentProvision(provision) {
  var result = await parent.Xrm.WebApi.retrieveRecord(
    "qm_rclegislation",
    provision._qm_rcparentlegislationid_value,
    `?$select=qm_name,qm_legislationlbl,qm_legislationetxt,qm_legislationftxt,_qm_tylegislationtypeid_value,_qm_rcparentlegislationid_value`
  );
  return result;
}
...
function sortProvisions(provisionArray) {
  ...
  hasOrderNumber.sort(function (a, b) {
    return a.ts_ordernbr - b.ts_ordernbr;
  });
