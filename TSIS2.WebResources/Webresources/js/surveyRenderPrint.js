...
function InitializeSurveyRender(surveyDefinition, surveyResponse, surveyLocale, mode) {
  ...
  survey.onAfterRenderQuestion.add(function (survey, options) {
    // When printing questions only, we need to hide the answer/input parts of certain widgets.
    if (window.questionsOnly === true) {
      if (options.question.getType() === "finding") {
        const el = options.htmlElement;
        el.querySelector(".operationsContainer")?.remove();
        el.querySelector(".inspectorComments")?.remove();
        el.querySelector(".character-count")?.remove();
        el.querySelector('label[for="comment"]')?.remove();
      }
...
function convertTextAreasToDivs() {
  $("textarea").each(function () {
    ...
    var newDiv = $('<div class="printed-textarea"></div>').html(content);
