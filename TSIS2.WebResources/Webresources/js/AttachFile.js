  ...
  parent.Xrm.WebApi.retrieveMultipleRecords(PrimaryTypeEntityName, "?fetchXml=" + encodedRecordOwnerFetchXML).then(
    async function success(result) {
      ...
      if (result.entities[0] != undefined) {
        ...
        fileUploadData.recordOwnerBUId = result.entities[0].recordOwnerBUId || null;
  ...
  switch (entityName) {
    ...
    case "ts_action":
      if (fileUploadData.recordOwnerTeamId) {
        const ownerValue = [{ id: fileUploadData.recordOwnerTeamId }];
        const isAvSec = await isOwnedByAvSec(ownerValue);
        const isISSO = await isOwnedByISSO(ownerValue);
