...
var MainForm =  (function () {
    ...
    MainForm.workOrderTypeOnChange = function (eContext) {
        try {
            var form = eContext.getFormContext();
    ...
    MainForm.fiscalYearOnChange = function (eContext) {
        this.removeSelectedFiscalQuarter(eContext);
    };
    MainForm.operationTypeOnChange = function (eContext) {
        try {
            var form = eContext.getFormContext();
            var workOrderTypeAttribute = form.getAttribute("msdyn_workordertype");
            var regionAttribute = form.getAttribute("ts_region");
    ...
    MainForm.siteOnChange = function (eContext) {
        try {
            ...
            if (siteAttribute != null && siteAttribute != undefined) {
                ...
                if (siteAttributeValue != null && siteAttributeValue != undefined &&
                    ...
                    Xrm.WebApi.retrieveMultipleRecords("msdyn_customerasset", "?fetchXml=" + encodedFetchXml).then(function success(result) {
                        ...
                        $this_1.showErrorMessageAlert(error);
                    });
                }
            }
        }
    ...
    MainForm.systemStatusOnChange = function (eContext) {
        var form = eContext.getFormContext();
        var systemStatus = form.getAttribute("msdyn_systemstatus").getValue();
    ...
    MainForm.caseOnChange = function (eContext) {
        var form = eContext.getFormContext();
    ...
    MainForm.stateCodeOnChange = function (eContext) {
        var form = eContext.getFormContext();
    ...
    MainForm.updateCaseView = function (eContext) {
        try {
            ...
            if (caseAttribute != null && caseAttribute != undefined) {
                if (caseAttributeValue != null) {
                    ...
                    Xrm.WebApi.online.retrieveRecord("incident", caseAttributeValue[0].id.replace(/({|})/g, ''), "?$select=_ovs_region_value, _ts_country_value, _customerid_value, _msdyn_functionallocation_value").then(function success(result) {
                        ...
                        $this_2.showErrorMessageAlert(error);
                    });
                }
    ...
    MainForm.showErrorMessageAlert = function (error) {
        var alertStrings = { text: error.message };
        var alertOptions = { height: 120, width: 260 };
        Xrm.Navigation.openAlertDialog(alertStrings, alertOptions).then(function () { });
    };
    MainForm.setDefaultFiscalYear = function (form) {
    ...
    MainForm.removeSelectedFiscalQuarter = function (eContext) {
        var form = eContext.getFormContext();
        form.getAttribute('ovs_fiscalquarter').setValue(null);
    };
    MainForm.setRegion = function (form) {
    ...
    MainForm.setOperationTypeFilteredView = function (form, regionAttributeId, countryCondition, workOrderTypeAttributeId) {
        form.getControl("ovs_assetcategory").setDisabled(false);
    ...
    MainForm.closeWorkOrderServiceTasks = function (formContext, workOrderServiceTaskData) {
        var $this = this;
    ...
    MainForm.setWorkOrderServiceTasksView = function (form, active) {
        var workOrderView;
        if (active) {
            workOrderView =
                {
