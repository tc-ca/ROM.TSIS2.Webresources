...
namespace ROM.WorkOrder {
    ...
    export function subSubSiteOnChange(eContext: Xrm.ExecutionContext<any, any>): void {
        try {
            ...
            const stakeholderAttribute = form.getAttribute("msdyn_serviceaccount");
            const siteAttribute = form.getAttribute("ts_site");
            const subSiteAttribute = form.getAttribute("msdyn_functionallocation");//ts_subsite on Operation entity
                        ...
                        function success(result) {
                            if (result.entities.length > 0) {
                                ...
                                if (targetSubSiteIds != "") {
                                    ...
                                    form.getControl('msdyn_functionallocation').setVisible(true);
                                    form.getAttribute("msdyn_functionallocation").setRequiredLevel("required");//none,required
                                    var viewId = '{1B59589F-F122-5428-4771-79BC925240C3}';
                                    var entityName = "msdyn_functionallocation";
                                    var viewDisplayName = Xrm.Utility.getResourceString("ovs_/resx/WorkOrder", "FilteredSites");
                                    var activityTypeFetchXml = '<fetch no-lock="false"><entity name="msdyn_functionallocation"><attribute name="statecode"/><attribute name="msdyn_functionallocationid"/><attribute name="msdyn_name"/><filter><condition attribute="msdyn_functionallocationid" operator="in">' + targetSubSiteIds + '</condition></filter><order attribute="msdyn_name" descending="false"/></entity></fetch>';
    ...
    function setActivityTypeFilteredView(form: Form.msdyn_workorder.Main.ROMOversightActivity, operationAttributeId: string, workOrderTypeAttributeId: string, operationTypeAttributeId: string): void {
        ...
        const opTypeId = operationTypeAttributeId ? operationTypeAttributeId.replace(/[{}]/g, "") : operationTypeAttributeId;

        Xrm.WebApi.retrieveRecord("ovs_operationtype", opTypeId, "?$select=_owningbusinessunit_value").then(
            async function success(operationType) {
                let operationActivityFilter = "";

                if (!isFromSecurityIncident && operationType._owningbusinessunit_value) {
                    const isAvSec = await isAvSecBU(operationType._owningbusinessunit_value);
                    if (isAvSec) {
     ...
     */
    async function isCurrentUserAvSecDomestic(): Promise<boolean> {
        try {
    ...
    */
     ...
     * @returns {void}
     */
    async function showWorkOrderRationaleByBusinessUnit(form: Form.msdyn_workorder.Main.ROMOversightActivity, isUnplannedCategory: boolean): Promise<void> {
        try {
     ...
     * @returns {void}
     */
    function showWorkOrderJustificationField(form: Form.msdyn_workorder.Main.ROMOversightActivity): void {
