namespace ROM.WorkOrderServiceTask {
  ...
  export function ToggleQuestionnaire(eContext: Xrm.ExecutionContext<any, any>): void {
    ...
    const wrCtrl = <Xrm.WebResourceControl>Form.getControl('WebResource_QuestionnaireRender');
    const questionnaireDefinition = (<Xrm.Attribute<string>>Form.getAttribute('ovs_questionnairedefinition')).getValue();
    const questionnaireResponse = (<Xrm.Attribute<string>>Form.getAttribute('ovs_questionnairereponse')).getValue();
  ...
  export function onSave(eContext: Xrm.ExecutionContext<any, any>): void {
    ...
    // Get the web resource control on the form
    const wrCtrl = <Xrm.Control<any>>Form.getControl('WebResource_QuestionnaireRender');
